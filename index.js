...
app.put('/updateGroupSettle', (req, res) => {
    ...
    db.query(
        "UPDATE GROUP_TABLE SET status = ? WHERE group_id = ?",
        [statusValue, group_id],
        (err, result) => {
            if (err) {
                console.error("MySQL Error:", err);
            ...
            res.json({ 
                success: true,
                message: `Group settlement status updated to ${status ? 'settled' : 'unsettled'}`,
                result
            });
        }
    );
